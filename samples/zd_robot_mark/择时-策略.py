...
class MyStrategy(bt.Strategy):
    ...
    def timing_model(self):
        ...
        # 单均线通道 03180718sgcn01
        adam_df = get_strategy_signal("03180718sgcn01",timing_date,timing_date)

        starttime = time.time()
        bft_df = pd.DataFrame(
            {
                "建议仓位": adam_df.position_rate_399001.values[0]*100,
                "指数": '399001',
                "计算时间": timing_date,
            },
            index=[0],
        ...
        endtime = time.time()
        bft_df["步骤耗时N秒"] = endtime - starttime
    ...
    def values_trade_plane(self, data):
        ...
        if data._name.split("_")[-1] == "电子I" and (
            ...
            trade_plan = True
        elif data._name.split("_")[-1] == "家用电器I" and (
            data.peg < 1
        ...
        elif data._name.split("_")[-1] == "电气设备I" and (
    ...
    def rebalance_portfolio(self):
        ...
        if self.p.timingprice:
            timingdata = self.timing_model() / 100
        else:
            timingdata = self.timing_data
        # 预计剩余现金 = 总资产 - 不清仓的市值
        self.p_value = (total_value * timingdata - last_ranks_values) / len(
            self.long_ranks
        )
        if self.p_value < 0:
            ...
            if timingdata == 0:
                ...
                # 同比例减仓
                # 当前持有仓位的股票列表
                posdata = [d for d, pos in self.getpositions().items() if d in self.ranks]
                reduce_value = -(total_value * timingdata - last_ranks_values)/len(posdata)
                for data in self.long_ranks:
                    # 按照次日开盘价计算下单量,下单量是100的整数倍
                    if len(data) < data.buflen():
                        size = int(reduce_value / 100 / data.open[1]) * 100
    ...
    def rebalance_goodprice(self, data):
        """
        调仓日执行的好价格逻辑
        Returns:
...
def test_start():
    ...
    for ye in ["20180321"]:
        ...
        tmc_data = (
            macroscopic_data[macroscopic_data.CODES == "总市值"]
            .fillna(method="pad")
            .query("DATES >= @start")
        )
        gdp_data = (
            macroscopic_data[macroscopic_data.CODES == "国内生产总值"]
            .fillna(method="pad")
            .query("DATES >= @start")
        )
        ta_data = (
            macroscopic_data[macroscopic_data.CODES == "央行总资产"]
            .fillna(method="pad")
            .query("DATES >= @start")
        )
        ...
        for vlnum in [50]:
            ...
            goodprice = True
            timingprice = True
            res_file_name = f"C:/Users/32771/Desktop/回测/择时/{strategy_name}-{vlnum}-{str(riskprice)}-{str(goodprice)}-{str(timingprice)}"
            # 添加策略
            cerebro.addstrategy(
                MyStrategy,
            ...
            df1.to_excel(f"{res_file_name}-绩效指标.xlsx")
            # 绩效指标
            # df00.绩效指标.replace({
            #     'calmar_ratio':'卡玛比率',
