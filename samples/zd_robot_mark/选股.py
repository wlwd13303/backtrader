...
def test_start():
    ...
    for ye in ["20180321"]:
        ...
        for vlnum in [50]:
            ...
            riskprice = False
            goodprice = True
            res_file_name = f"C:/Users/32771/Desktop/回测/周末展示4/{strategy_name}-{vlnum}-{str(riskprice)}-{str(goodprice)}"
            # 添加策略
            cerebro.addstrategy(
                MyStrategy,
...
def save_csv(name, start, end):
    ...
    if name[:6] != "000001":
        try:
            ...
            data_values = pd.read_csv(
                f"C:/Users/32771/Desktop/回测/实盘/估值分位/"
                f"{name[:6]}.csv"
            )
